...
<script type="text/javascript">
	$(function() {
		$('#dg_par3').datagrid({
			url : '/signer32/Manager/Manager/getManagers',
			method : 'get',
			lines : true,
			rownumbers : true,
			idField : 'id',
	...
	function tb3_add() {
		...
		$('#tb3_add_form').form({
			url : '/signer32/Manager/Manager/add',
			onSubmit : function() {
	...
	function tb3_del() {
		...
		if (row) {
			...
			$.messager.confirm('删除人员', '确定删除？', function(r) {
				if (r) {
					// 退出操作;
					$.post("/signer32/Manager/Manager/delete", {
						mng_id : row['id']
					}, function(data) {
						if (data['status']) {
		...
		if (editingId3 != undefined) {
			$.messager.confirm('保存', '是否保存？', function(r) {
				if (r) {
					...
					$.post("/signer32/Manager/Manager/update", {
						mng_id : row['id'],
						mng_uid : row['uid'],
						mng_name : row['name'],
						mng_sex : row['sex'],
	...
	function tb3_listdep(){
		row = $('#dg_par3').datagrid('getSelected');
		if(row){
			$('#listdep').window('refresh','/signer32/Manager/Manager/listdep?mng_id='+row['id']);
			$('#listdep').window('open');
		}
	}
</script>
