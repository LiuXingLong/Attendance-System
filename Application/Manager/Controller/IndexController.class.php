...
class IndexController extends BaseController {
	public function index() {
		$news = M("News");
		$res = $news->order("time DESC")->limit("0,10")->where(array("type"=>2,"type_id"=>$_SESSION['mng_id']))->field("id,title,time,content")->select();
	...
	public function getSysInfo() {
		$m = M ( "Manager" );
		$res = $m->field("mng_uid,mng_name,mng_email")->where(array("mng_id"=>$_SESSION['mng_id']))->select();
		$d = new \Org\Util\Date ();
		$res ['sys_time'] = $d->format ( "%Y-%m-%d %H:%M" );
		$res ['sys_tenant_total'] = $res[0]['mng_name'];
		$res ['sys_tenant_isrun'] = $res[0]['mng_email'];
		
		$res ['sys_time_bj'] = gmdate ( "Y-n-j H:i", time () + 8 * 3600 );
		$res ['sys_env'] = substr ( $_SERVER ["SERVER_SOFTWARE"], 0, 12 );
		$res ['sys_os'] = $res[0]['mng_uid'];
